...
Set `package` to the full HTTPS download URL of a ready-to-run ZIP in the current space's private GitLab Generic Package registry, and set `packageSha256` to its 64-character lowercase SHA-256 digest. A composed Job reads the registry credentials distributed to each node by `registry-operator`, verifies the download and ZIP structure, and deploys it with Azure CLI through the cluster's existing managed identity. Change the immutable URL and digest to roll out a new package. Omitting both fields leaves the Function ready for a manual OneDeploy workflow.
